...
export class ManageRooms implements OnInit {
  ...
  saveRoom() {
    ...
    // Logic to handle available rooms
    if (!this.isEditing) {
      // For a new room type, initially all rooms are available
      this.currentRoom.availableRooms = this.currentRoom.totalRooms;
    } else {
      // Ensure available rooms do not exceed total rooms if owner decreased total rooms
